    ...
    public class TafelArray
    {
        ...
        public TafelArray() { } //Empty constructor for json deserializen
           ...
           public void BekijkVrijeTafels(string tijdEnDatum) //Todo: tafels zijn ook niet vrij als ze een aantal uur voor de gegeven tijd bezet zijn.
          {
            ...
            string checkDatum = tijdEnDatumNaarDatum(tijdEnDatum); //De gegeven datum in string (Zaterdag 1 januari 2021)
               ...
               foreach (Tafel tafel in Tafels)
               {
                ...
                if (tafel.Gereserveerd != null)
                {
                    ...
                    {
                        //Console.WriteLine($"{tijdEnDatumNaarDatum(data)}, {checkDatum}");
                        //Console.ReadLine();
           ...
           public int tijdEnDatumNaarInt(string tijdEnDatum)
        {
            int tijd = int.Parse(tijdEnDatum.Substring(0,4)); //pakt de eerste 4 characters van een tijd (1200)
            return tijd;
        }

        public string tijdEnDatumNaarDatum(string tijdEnDatum)
        {
            string datum = tijdEnDatum.Substring(4); //pakt de characters na de tijd (datum)
            return datum;
        }

        public bool tijdCheck(int tafelTijd, int reserveringTijd, int uren) 
        //Als tafeltijd (1200) + uren (200) is groter dan reserveringtijd (1324), of als reserveringstijd (1324) - uren (200) is kleiner of gelijk aan tafeltijd (1200) returnt true, anders false.
        {
            Console.WriteLine($"Tafeltijd:{tafelTijd}, reserveringstijd: {reserveringTijd}, uren: {uren}");
            Console.ReadLine();
            if ((tafelTijd + uren >= reserveringTijd) || ((reserveringTijd - uren) <= tafelTijd))
            {
                //Console.WriteLine("Al gereserveerd");
                //Console.ReadLine();
                return true;
            }
            return false;
